...
export default function Men() {
  ...
  const { data: productsData, isLoading } = useProducts({ page: currentPage, category_id: 1 });
  ...
  const uniqueTypes = [...new Set(menProducts.map(p => getProductType(p)).filter(Boolean))];
  const menClothingTypes = uniqueTypes.length > 0 ? uniqueTypes : ['tshirt', 'jeans', 'jacket'];
      ...
      <section className="py-16 px-6 lg:px-8 max-w-7xl mx-auto">
        <div className="mb-10">
          <CategoryFilter
            ...
            availableTypes={menClothingTypes}
          />
        </div>
